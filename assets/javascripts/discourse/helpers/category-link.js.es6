...
function subCategoryStripe(color, classes) {
  var style = color ? "style='border: 2px solid #" + color + ";'" : "";
  return "<span class='" + classes + "' " + style + "></span>";
}
...
export function categoryBadgeHTML(category, opts) {
  ...
  if (!opts.hideBadge) {
    if (get(category, 'parent_category_id')) {
      html += subCategoryStripe(color, "badge-subcategory-bg");
    } else {
...
export function categoryLinkHTML(category, options) {
  var categoryOptions = { hideParent: true };
  ...
  if (options) {
    ...
    if (options.hideBadge) { categoryOptions.hideBadge = true; }
  }
