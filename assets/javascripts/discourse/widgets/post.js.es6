...
import { transformBasicPost } from 'discourse/plugins/discourse-idnet-template/discourse/lib/transform-post';
import { h } from 'virtual-dom';
import DiscourseURL from 'discourse/lib/url';
import { dateNode } from 'discourse/plugins/discourse-idnet-template/discourse/helpers/node';
...
createWidget('post-avatar', {
  ...
  html(attrs) {
    ...
    const posterName = [this.attach('poster-name', attrs)];
    const joinedAt = h('div.joined-at', 'Joined at ' + (new Date(attrs.userJoinedAt).getFullYear()));
    const postsCount = h('div.posts-count', attrs.userPostCount + ' Posts');
    return [body, posterName, h('div.poster-avatar-extra'), joinedAt, postsCount];
  }
});
...
createWidget('post-contents', {
  ...
  html(attrs, state) {
    ...
    result.push(new PostCooked(attrs, new DecoratorHelper(this)));
