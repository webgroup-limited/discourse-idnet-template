...
export default createWidget('poster-name', {
  ...
  html(attrs) {
    ...
    const contents = [h('div', { className: classNames.join(' ') }, nameContents)];
    if (name && this.siteSettings.display_name_on_posts && sanitizeName(name) !== sanitizeName(username)) {
      contents.push(h('div.second.' + (nameFirst ? "username" : "full-name"),
    ...
    if (title && title.length) {
      ...
      contents.push(h('div.user-title', titleContents));
    }
